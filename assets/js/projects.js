const projects = [
  ...
  },
  {
    ...
    id: "ubc",
    githubLink: undefined,
    siteLink: "https://artsone.arts.ubc.ca/student-journal/songs-of-innocence-and-of-experience-a-heaven-of-misery/",
  },
  {
...
// Create Project Cards and Modals with a single function using a project object
function createProjectElements(project) {
  const { title, text, imageSrc, id, githubLink, siteLink } = project;
  ...
  let githubButton = githubLink
    ? `
    <a href="${githubLink}" target="_blank" class="btn">
        <span class="btn-unselected"><i class="bi bi-box-arrow-up-right"></i> GitHub</span>
...
const populateProjects = () => {
  projects.forEach((project) => {
    const { cardHtml, modalHtml } = createProjectElements(project);
    document.getElementById(`${project.id}CardContainer`).innerHTML = cardHtml;
    document.getElementById(`${project.id}ModalContainer`).innerHTML = modalHtml;
  });
};
